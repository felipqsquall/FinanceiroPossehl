...
public class BuildRequestPut {
    public static String build(BigDecimal nuFin, BigDecimal recDesp) throws Exception {

        //Trecho que trata a Renegociação
        if(recDesp.equals(BigDecimal.ZERO)) {
            JapeWrapper daoFinanceiro = JapeFactory.dao("AD_VGFRENEG");
        ...
        //Trecho que trata baixa do Título
        else {

            JapeWrapper daoFinanceiro = JapeFactory.dao("AD_VGFBAIXADO");
            DynamicVO financeiroVO = daoFinanceiro.findByPK(nuFin);
            if (financeiroVO == null) {
                return null;
            }
